...
> 状态：**待批准 / v0.1**
> 本文件已经从占位草案收束为可执行总纲；用户明确批准前不得开始批量实现。
...
- UI：Qt Quick/QML；
- 构建：CMake + Ninja；
- 正式候选：Qt 6.8 LTS + MSVC 2022 x64；
- 当前可复现验证环境：Qt 6.9.0 MinGW x64、CMake 3.30.5、Ninja 1.12.1、GCC 13.1；
- 数据库：Qt SQL + SQLite；
- 初始媒体后端：Qt Multimedia；
- 发布：`windeployqt` + CPack/Inno Setup，GitHub Releases 仅发布 x64 Setup EXE。

M5 前必须固定并验证正式发布工具链。当前 MinGW 环境用于立即可执行的开发验证，不取消 MSVC 候选。
