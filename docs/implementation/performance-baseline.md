...
|---|---:|---:|---:|---:|
| Release 空闲启动（单次） | 27.35 MiB | 38.74 MiB | 250 | 手动结束采样 |
| Release Smoke 启动退出（单次） | 不采样 | 不采样 | 不采样 | 523 ms，退出码 0 |

这是空骨架基线，不代表完整产品的内存结论。当前场景没有加载 SourceHost，进程树只有主程序。M5 需要在同一设备上至少重复三次，并增加 SourceHost 已加载、播放本地音频和歌词场景，报告进程树中位数与最大值。
