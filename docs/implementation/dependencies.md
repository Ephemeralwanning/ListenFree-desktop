...
|---|---|---|---|---|---|
| Qt | 6.10.3 | Core、Gui、QML、Quick、SQL、Network、Multimedia、Concurrent、Test、ShaderTools | 系统外部 Qt kit，通过 CMake `find_package` | 由实际部署模块决定；不引入 WebEngine/Chromium | Qt 6.8 LTS（发布分支候选） |
...
| aqtinstall | 3.3.0 | 用户级安装 Qt 6.10.3 kit | Python 用户包，安装期使用 | 不进入应用包或运行时 | Qt Maintenance Tool（当前企业账户认证失败） |
