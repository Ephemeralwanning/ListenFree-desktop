...
bool OutputNull::initialize(quint32 freq, ChannelMap map, Qmmp::AudioFormat format)
{
    ...
    capture_.setFileName(qEnvironmentVariable("LISTENFREE_TEST_PCM"));
    if(!capture_.open(QIODevice::WriteOnly|QIODevice::Append))return false;
    QFile info(capture_.fileName()+".format");if(info.open(QIODevice::Append))info.write(QByteArray::number(freq)+","+QByteArray::number(map.count())+","+QByteArray::number(int(format))+"\n");
...
{
   capture_.write(reinterpret_cast<const char*>(data), maxSize); capture_.flush();
